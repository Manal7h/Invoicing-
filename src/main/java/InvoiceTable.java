...
public class InvoiceTable {
	public static void InvoiceTable() throws IOException {
		...
		String sql = "CREATE TABLE InvoiceTable" + "(Invoice_Id INTEGER PRIMARY KEY IDENTITY(1,1) ,"
				+ " Invoice_No INTEGER NOT NULL ," + "Invoice_Date date NOT NULL ," + "No_of_items INTEGER NOT NULL ,"
				+ "Total decimal(10,2), " + "Paid_Amount decimal(10,2)," + " Balance decimal(10,2),"
				+ "Customer_Id INTEGER REFERENCES CustomerTable(Customer_Id),"
				+ "Item_ID INTEGER REFERENCES ItemTable(Item_ID)," + "Shop_Id INTEGER REFERENCES Shop(Shop_Id),"
				+ " Header_Id INTEGER REFERENCES InvoiceHeader(Header_Id))";
...
//	    String pass = "root";
		...
		String sql = "insert into InvoiceTable values(" + Invoice_No + ",  '" + Invoice_Date + "'," + No_of_items
				+ ",  " + Total + ", " + Paid_Amount + ", " + Balance + "," + Customer_Id + ", " + Item_ID + ", "
				+ Shop_Id + ", " + Header_Id + ")";
